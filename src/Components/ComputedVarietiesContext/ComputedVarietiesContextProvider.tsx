...
function computeProductValues(
  ...
  return products.map(product => {
    ...
    const growthThroughput = Math.floor(pots * HOURS_IN_DAY / GROW_TIME); // max batches by transforming
    ...
    return {
      ...
      timePerBatch: GROW_TIME + (MIX_TIME * mixSteps), // gathering + transforming + mixing
...
const ComputedVarietiesProvider = ({varieties, property, customers, defaultPotsUsed, defaultMixingMachinesUsed, children}: ComputedVarietiesProviderProps) => {
    ...
    const computedVarieties: {varieties: ComputedVariety[], isSuggested: boolean, mixingMachineUsed: number, potsUsed: number } = useMemo(() => {
      ...
      //     v.seedCost +
      //     (v.ingredientCost * 8) +
      //     (60 / 30); // Assuming "High Quality" soil
      //   const timeToGrowInHours = 15;
      //   const totalProfit = v.budPrice * 8; // Assuming 8 buds per batch
      ...
      //   const mixStepsMixingMachineDiff = v.mixSteps.length - (mixingMachines || 1); // assuming at least 1 mixing machine
      //   const timePerBatch = (6 * v.mixSteps.length * 8) / 60;
      //   const maxBatchGrowthPerDay = (24 / timeToGrowInHours) * (pots || 8); // we can grow 24 hours a day, but we need to take into account the number of pots we have
      //   const maxBatchFullMixPerDay = Math.min(24 / ((timePerBatch / (mixingMachines || 1)) + (mixStepsMixingMachineDiff > 0 ? timePerBatch / mixStepsMixingMachineDiff : 0)), maxBatchGrowthPerDay); // if mixStepsMixingMachineDiff is more than 0, we need to reuse some mixing machines because we have more mix steps than machines
      //   const maxProductionPerDay = Math.min(maxBatchGrowthPerDay, maxBatchFullMixPerDay); // we can only mix as much as we can grow
      //   const customerDemand = totalAverageOrderQuantity;
      //   const maxProductionPerDayLimited = Math.min(maxProductionPerDay * 5, customerDemand);
      //   const profitPerDay = maxProductionPerDayLimited * profitPerBud;
      //   return { ...v, totalCost, profitPerBatch, profitPerBud, ratioCostBenefit, timePerBatch, profitPerDay, mixSteps: v.mixSteps.length, maxProductionPerDay };
      // });
